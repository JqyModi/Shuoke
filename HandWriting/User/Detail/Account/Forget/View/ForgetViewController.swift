...
class ForgetViewController: UIViewController {
    ...
    @objc private func keyBoardDidChangeFrame(notification: Notification) {
        ...
        let loginY = forgetView?.frame.maxY
        //判断键盘是否覆盖
        var y: CGFloat = 0
        if loginY! > keyBY {
            debugPrint("遮挡状态")
            y = (forgetView?.frame.maxY)! - keyBoardFrame.minY
            //通过动画方式改变改变loginView的Y值
            UIView.animate(withDuration: 0.6) {
                //            self.loginView?.transform = CGAffineTransform(translationX: 0, y: -y1)
                self.forgetView?.frame = CGRect(x: (SCREEN_WIDTH-300)/2, y: y, width: 300, height: 300)
            }
        }else {
            debugPrint("不遮挡状态")
    ...
    private func handleEvent() {
        ...
        center.addObserver(self, selector: #selector(ForgetViewController.keyBoardDidChangeFrame(notification:)), name: NSNotification.Name.UIKeyboardDidChangeFrame, object: nil)
