...
class CopyBookPresenter: BothamPresenter, BothamPullToRefreshPresenter, BothamNavigationPresenter{
    //初始化
    ...
    func viewDidLoad() {
         ...
         */
        getHomeCommonData(type: "beitie", pid: 0, page: currentPage) { [weak self](items) in
            //开始填充数据
            self?.data = items
...
//        CacheCommonService.selectCommonsByType(type: 2, page: 1) { [weak self] (commons) in
...
//        }
    ...
    func didStartRefreshing() {
        ...
        /*
         联网获取数据
         1、http://shuoke360.cn/api/copybook/?token=gstshuoke360&page=1&type=yuedu&pid=411
         type=beitie   pid= ""      每日推荐
         */
//        getHomeCommonData(type: "beitie", pid: 0, page: 1) { [weak self](items) in
//            //开始填充数据
//            self?.data = items
//            self?.load(items: (self?.data)!)
//        }
        ...
        CacheCommonService.selectCommonsByType(type: 2, page: 1) { [weak self] (commons) in
            self?.load(items: commons)
            debugPrint("**********cachecount = \(commons.count)")
            CacheCommonService.insertCommonWithArray(arr: commons, type: 2)
        }
